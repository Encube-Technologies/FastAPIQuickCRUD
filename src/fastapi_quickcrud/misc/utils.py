...
from .crud_model import CRUDModel, RequestResponseModel
from .exceptions import PrimaryMissing, QueryOperatorNotFound, UnknownColumn
...
def find_query_builder(param: dict, model: Base) -> List[Union[BinaryExpression]]:
    ...
    for column_name, value in param.items():
        if (
            ExtraFieldType.Comparison_operator in column_name
            or ExtraFieldType.Matching_pattern in column_name
        ):
            continue
        ...
        if not operators:
            raise QueryOperatorNotFound(
                f"The query operator of {column_name} not found!"
            )
        if not isinstance(operators, list):
            operators = [operators]
        for operator in operators:
            sub_query.append(
                process_map[operator](getattr(model, table_column_name), value)
            )
...
def sqlalchemy_to_pydantic(
    ...
    *,
    ...
    exclude_columns_request: List[str] = None,
    exclude_columns_response: List[str] = None,
    body_url_variable_mapping: dict[str, str] = None,
    constraints=None,
    ...
    @param exclude_columns_request str List[str]:
        Excluded columns from request based on sqlalchemy model

    @param exclude_columns_response List[str]:
        Excluded columns from response based on sqlalchemy model

    @param  body_url_variable_mapping Dict[str, str]:
        Key value map of body key and python string to be evaluated to add to request
        Possible to add values from reuqest headers, request url params or for example internal values from authentication

        Usage:
            body_url_variable_mapping = {"account_id": 'request.path_params["account__pk__account_id"]'}

        Outcome:
            Function will add value from url param "account__pk__account_id" to api request body under key "account_id".
    ...
    if exclude_columns is None:
        exclude_columns = []
    if exclude_columns_request is None:
        exclude_columns_request = []
    if exclude_columns_response is None:
        exclude_columns_response = []
    if foreign_include is None:
        foreign_include = {}
    request_response_mode_set = {}
    model_builder = ApiParameterSchemaBuilder(
        db_model,
    ...
    REQUIRE_PRIMARY_KEY_CRUD_METHOD = [
        CrudMethods.DELETE_ONE.value,
    ...
    for crud_method in crud_methods:
        ...
        if (
            crud_method.value in REQUIRE_PRIMARY_KEY_CRUD_METHOD
            and not model_builder.primary_key_str
        ):
            raise PrimaryMissing(
                f"The generation of this API [{crud_method.value}] requires a primary key"
            )
...
process_map = {
    RangeFromComparisonOperators.Greater_than: lambda field, value: field > value,
    RangeFromComparisonOperators.Greater_than_or_equal_to: lambda field, value: field
    >= value,
    RangeToComparisonOperators.Less_than: lambda field, value: field < value,
    RangeToComparisonOperators.Less_than_or_equal_to: lambda field, value: field
    <= value,
    ItemComparisonOperators.Equal: lambda field, values: or_(
        field == value for value in values
    ),
    ItemComparisonOperators.Not_equal: lambda field, values: or_(
        field != value for value in values
    ),
    ItemComparisonOperators.In: lambda field, values: or_(field.in_(values)),
    ItemComparisonOperators.Not_in: lambda field, values: or_(field.notin_(values)),
    MatchingPatternInStringBase.case_insensitive: lambda field, values: or_(
        field.ilike(value) for value in values
    ),
    MatchingPatternInStringBase.case_sensitive: lambda field, values: or_(
        field.like(value) for value in values
    ),
    MatchingPatternInStringBase.not_case_insensitive: lambda field, values: or_(
        field.not_ilike(value) for value in values
    ),
    MatchingPatternInStringBase.not_case_sensitive: lambda field, values: or_(
        field.not_like(value) for value in values
    ),
    MatchingPatternInStringBase.contains: lambda field, values: or_(
        field.contains(value) for value in values
    ),
    PGSQLMatchingPatternInString.similar_to: lambda field, values: or_(
        field.op("SIMILAR TO")(value) for value in values
    ),
    PGSQLMatchingPatternInString.not_similar_to: lambda field, values: or_(
        field.op("NOT SIMILAR TO")(value) for value in values
    ),
    PGSQLMatchingPatternInString.match_regex_with_case_sensitive: lambda field, values: or_(
        field.op("~")(value) for value in values
    ),
    PGSQLMatchingPatternInString.match_regex_with_case_insensitive: lambda field, values: or_(
        field.op("~*")(value) for value in values
    ),
    PGSQLMatchingPatternInString.does_not_match_regex_with_case_sensitive: lambda field, values: or_(
        field.op("!~")(value) for value in values
    ),
    PGSQLMatchingPatternInString.does_not_match_regex_with_case_insensitive: lambda field, values: or_(
        field.op("!~*")(value) for value in values
    ),
}
...
def table_to_declarative_base(db_model):
    ...
    if not db_model.primary_key:
        db_model.append_column(
            Column("__id", Integer, primary_key=True, autoincrement=True)
        )
