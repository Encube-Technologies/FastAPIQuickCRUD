...
class SQLAlchemyGeneralSQLQueryService(ABC):
    ...
    def get_many(
        self, *, join_mode, query, target_model=None, abstract_param=None
    ) -> BinaryExpression:
        filter_args = query
        ...
        for c in inspect(self.model).relationships:
            for remote_pair in c.local_remote_pairs:
                if model.__table__ in [pair_part.table for pair_part in remote_pair]:
                    primary_join = c.primaryjoin

        filter_list: List[BinaryExpression] = find_query_builder(
            param=filter_args, model=model
        )
        path_filter_list: List[BinaryExpression] = path_query_builder(
            params=abstract_param, model=self.foreign_table_mapping
        )
        ...
        if order_by_columns:
            ...
            for order_by_column in order_by_columns:
                ...
                sort_column, order_by = (
                    order_by_column.replace(" ", "").split(":") + [None]
                )[:2]
                if not hasattr(self.model_columns, sort_column):
                    raise UnknownColumn(f"column {sort_column} is not exited")
                if not order_by:
                    order_by_query_list.append(
                        getattr(self.model_columns, sort_column).asc()
                    )
                elif order_by.upper() == Ordering.DESC.upper():
                    order_by_query_list.append(
                        getattr(self.model_columns, sort_column).desc()
                    )
                    ...
                    raise UnknownOrderType(
                        f"Unknown order type {order_by}, only accept DESC or ASC"
                    )
            if order_by_query_list:
                stmt = stmt.order_by(*order_by_query_list)
        stmt = stmt.limit(limit).offset(offset)
        stmt = self.get_join_by_excpression(stmt, self.model, join_mode=join_mode)
        return stmt

    def get_one(
        self, *, extra_args: dict, filter_args: dict, join_mode=None
    ) -> BinaryExpression:
        filter_list: List[BinaryExpression] = find_query_builder(
            param=filter_args, model=self.model_columns
        )

        extra_query_expression: List[BinaryExpression] = find_query_builder(
            param=extra_args, model=self.model
        )
        join_table_instance_list: list = self.get_join_select_fields(join_mode)
        model = self.model
        if not isinstance(self.model, Table):
    ...
    def create(
        ...
        *,
        ...
        insert_arg_dict: list[dict] = [
            clean_input_fields(model=self.model_columns, param=insert_arg)
    ...
    def insert_one(self, *, insert_args) -> BinaryExpression:
        insert_args = insert_args
        update_columns = clean_input_fields(insert_args, self.model_columns)
    ...
    def get_join_by_excpression(
        self, stmt: BinaryExpression, main_model, join_mode=None
    ) -> BinaryExpression:
        if not join_mode:
            return stmt
        for join_table, data in join_mode.items():

            if join_table == str(main_model.__table__):
                continue
            for local_reference in data["local_reference_pairs_set"]:
                ...
                reference_column = getattr(
                    local_reference["reference_table_columns"], reference
                )
    ...
    def model_query(
        ...
        *,
        ...
        if extra_args:
            filter_list += find_query_builder(
                param=extra_args, model=self.model_columns
            )
        stmt = select(self.model).where(and_(*filter_list))
        return stmt

    def get_one_with_foreign_pk(
        self, *, join_mode, query, target_model, abstract_param=None
    ) -> BinaryExpression:
        model = self.foreign_table_mapping[target_model]
        ...
        stmt = (
            select(*[model] + join_table_instance_list)
            .join(self.model, primary_join)
            .filter(and_(*filter_list + path_filter_list))
        )
...
class SQLAlchemyPGSQLQueryService(SQLAlchemyGeneralSQLQueryService):
    def __init__(self, *, model, async_mode, foreign_table_mapping):
        ...
        :param async_mode: bool
        """
        super(SQLAlchemyPGSQLQueryService, self).__init__(
            model=model,
    ...
    def upsert(
        ...
        *,
        ...
        if unique_fields and insert_with_conflict_handle:
            update_columns = clean_input_fields(
                insert_with_conflict_handle.__dict__.get("update_columns", None),
                self.model_columns,
            )
            if not update_columns:
                raise UpdateColumnEmptyException(
                    "update_columns parameter must be a non-empty list "
                )
            conflict_update_dict = {}
            for columns in update_columns:
                conflict_update_dict[columns] = getattr(insert_stmt.excluded, columns)

            conflict_list = clean_input_fields(
                model=self.model_columns, param=unique_fields
            )
            conflict_update_dict = clean_input_fields(
                model=self.model_columns, param=conflict_update_dict
            )
            insert_stmt = insert_stmt.on_conflict_do_update(
                index_elements=conflict_list, set_=conflict_update_dict
            )
