...
class SQLAlchemyGeneralSQLeResultParse(object):
    ...
    def _response_builder(sql_execute_result, fastapi_response, response_model):
        ...
        fastapi_response.headers["x-total-count"] = str(
            len(sql_execute_result) if isinstance(sql_execute_result, list) else "1"
        )
        return result
    ...
    @staticmethod
    def find_one_sub_func(
        sql_execute_result, response_model, fastapi_response, **kwargs
    ):
    ...
    async def async_find_one(
        self, *, response_model, sql_execute_result, fastapi_response, **kwargs
    ):
        result = self.find_one_sub_func(
            sql_execute_result, response_model, fastapi_response, **kwargs
        )
        await self.async_commit(kwargs.get("session"))
        return result
    ...
    async def async_find_many(
        ...
        result = self.find_many_sub_func(
            response_model, sql_execute_result, fastapi_response, **kwargs
        )
        await self.async_commit(kwargs.get("session"))
        return result
    ...
    async def async_create_one(
        self, *, response_model, sql_execute_result, fastapi_response, **kwargs
    ):
        result = self.create_one_sub_func(
            response_model, sql_execute_result, fastapi_response
        )
        await self.async_commit(kwargs.get("session"))
        return result
    ...
    async def async_create_many(
        ...
        result = self.create_many_sub_func(
            response_model, sql_execute_result, fastapi_response
        )
        await self.async_commit(kwargs.get("session"))
        return result
    ...
    async def async_upsert_one(
        self, *, response_model, sql_execute_result, fastapi_response, **kwargs
    ):
        result = self.upsert_one_sub_func(
            response_model, sql_execute_result, fastapi_response
        )
        await self.async_commit(kwargs.get("session"))
        return result
    ...
    async def async_upsert_many(
        ...
        result = self.upsert_many_sub_func(
            response_model, sql_execute_result, fastapi_response
        )
        await self.async_commit(kwargs.get("session"))
        return result
    ...
    def delete_many_sub_func(
        self, response_model, sql_execute_result, fastapi_response
    ):
        if not sql_execute_result:
            return Response(status_code=HTTPStatus.NO_CONTENT)
    ...
    def delete_many(
        self, *, response_model, sql_execute_results, fastapi_response, **kwargs
    ):
        session = kwargs.get("session")
        if sql_execute_results:
            for sql_execute_result in sql_execute_results:
                self.delete(session, sql_execute_result)
        result = self.delete_many_sub_func(
            response_model, sql_execute_results, fastapi_response
        )
        self.commit(session)
        return result
    ...
    def post_redirect_get_sub_func(
        self, response_model, sql_execute_result, fastapi_request
    ):
        result = parse_obj_as(response_model, sql_execute_result)
    ...
    def get_post_redirect_get_url(
        self, response_model, sql_execute_result, fastapi_request
    ):
        redirect_url = self.post_redirect_get_sub_func(
            response_model, sql_execute_result, fastapi_request
        )
        header_dict = {
            i[0].decode("utf-8"): i[1].decode("utf-8")
            for i in fastapi_request.headers.__dict__["_list"]
        }
        redirect_url += f"?{urlencode(header_dict)}"
        return redirect_url

    async def async_post_redirect_get(
        self, *, response_model, sql_execute_result, fastapi_request, **kwargs
    ):
        ...
        redirect_url = self.get_post_redirect_get_url(
            response_model, sql_execute_result, fastapi_request
        )
        await self.async_commit(session)
